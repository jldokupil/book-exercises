...
my_ui <- fluidPage(
  ...
  numericInput(inputId = "number1", label = "price", value = 0, min = 0),
  
  # A second `numericInput()` widget with the label "Quantity"
  # It should have a default value of 1 and a minimum value of 1  
  numericInput(inputId = "number2", label = "quantity", value = 1, min = 1),
...
server <- function(input_list, output_list){
  ...
  output_list$cost <- renderText({
    total <- input_list$number1 * input_list$number2
  })
